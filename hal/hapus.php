...
if(isset($_GET['id'])){
	...
	$cek = mysql_query("SELECT No_induk FROM siswa WHERE No_induk='$id'") or die(mysql_error());
	...
	if(mysql_num_rows($cek) == 0){
		...
		$del = mysql_query("DELETE FROM siswa WHERE No_induk='$id'");
		
		//jika query DELETE berhasil
		if($del){
			
			echo 'Data Mahasiswa berhasil di hapus! ';		//Pesan jika proses hapus berhasil
			echo '<a href="?page=mahasiswa">Kembali</a>';	//membuat Link untuk kembali ke halaman beranda
